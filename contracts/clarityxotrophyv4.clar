;;
;;  ClarityXO TROPHY NFT CONTRACT - v4
;;  clarityxotrophyv4.clar
;;
;;  Changes over v3:
;;    - Expanded string-ascii to 256 chars for full IPFS URLs
...
(define-non-fungible-token clarityxotrophyv4 uint)
    ...
    (match (stx-transfer? fee player contract-owner)
      success
        (let (
          ...
          (nft-mint? clarityxotrophyv4 token-id player)
        )
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? clarityxotrophyv4 token-id))
)
...
(define-public (transfer
  ...
  (begin
    ...
    (asserts! (is-some (nft-get-owner? clarityxotrophyv4 token-id)) err-not-token-owner)
    (asserts!
      (is-eq (nft-get-owner? clarityxotrophyv4 token-id) (some sender))
      err-not-token-owner)
    (nft-transfer? clarityxotrophyv4 token-id sender recipient)
  )
)
